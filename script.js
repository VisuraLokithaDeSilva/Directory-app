...
function showSuggestions() {
    ...
    if (input.length > 0) {
        const suggestions = data
            .filter(item => item[mode]?.toLowerCase().trim().includes(input))
        ...
        suggestions.forEach((suggestion, index) => {
            ...
            div.innerText = suggestion[mode].trim(); // Remove extra spaces
...
function displayResults(person) {
    ...
    const suggestionsDiv = document.getElementById("suggestions");
    suggestionsDiv.style.display = "none"; // Hide the suggestions when a result is selected
